...
import SharkeyAPI from './api_client.js'
import { UnknownNotificationTypeError } from '../notification.js'
import { isBrowser } from '../default.js'

/**
 * WebSocket
 * Sharkey is not support http streaming. It supports websocket instead of streaming.
 * So this class connect to Sharkey server with WebSocket.
 */
export default class WebSocket extends EventEmitter implements WebSocketInterface {
  ...
  /**
   * @param url Full url of websocket: e.g. wss://sharkey.example/streaming
  ...
  private _setupParser() {
    this.parser.on('update', (note: SharkeyAPI.Entity.Note) => {
      this.emit('update', SharkeyAPI.Converter.note(note))
    })
    this.parser.on('notification', (notification: SharkeyAPI.Entity.Notification) => {
      const n = SharkeyAPI.Converter.notification(notification)
    ...
    this.parser.on('conversation', (note: SharkeyAPI.Entity.Note) => {
      this.emit('conversation', SharkeyAPI.Converter.noteToConversation(note))
    })
...
export class Parser extends EventEmitter {
  ...
  public parse(ev: WS.MessageEvent, channelID: string) {
    ...
    switch (body.type) {
      case 'note':
        this.emit('update', body.body as SharkeyAPI.Entity.Note)
        break
      case 'notification':
        this.emit('notification', body.body as SharkeyAPI.Entity.Notification)
        break
      case 'mention': {
        const note = body.body as SharkeyAPI.Entity.Note
