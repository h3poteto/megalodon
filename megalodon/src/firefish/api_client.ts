	...
	export namespace Converter {
		...
		export const note = (n: Entity.Note): MegalodonEntity.Status => {
			return {
				...
				emojis: Array.isArray(n.emojis)
					? n.emojis.filter((e) => !e.name.includes("@")).map((e) => emoji(e))
					: [],
		...
		export const notification = (
			...
			if (n.reaction) {
				const reactions = mapReactions(n.note?.emojis ?? [], {
					[n.reaction]: 1,
				});
				...
				configuration: {
					statuses: {
						...
						max_media_attachments:
							i.configuration.statuses.max_media_attachments,
						characters_reserved_per_url:
							i.configuration.statuses.characters_reserved_per_url,
					},
					polls: {
						max_options: i.configuration.polls.max_options,
						max_characters_per_option:
							i.configuration.polls.max_characters_per_option,
