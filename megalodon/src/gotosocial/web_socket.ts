...
import GotosocialAPI from './api_client.js'
...
export default class Streaming extends EventEmitter implements WebSocketInterface {
  ...
  private _setupParser() {
    this.parser.on('update', (status: GotosocialAPI.Entity.Status) => {
      this.emit('update', GotosocialAPI.Converter.status(status))
    })
    this.parser.on('notification', (notification: GotosocialAPI.Entity.Notification) => {
      const n = GotosocialAPI.Converter.notification(notification)
    ...
    this.parser.on('status_update', (status: GotosocialAPI.Entity.Status) => {
      this.emit('status_update', GotosocialAPI.Converter.status(status))
    })
...
export class Parser extends EventEmitter {
  ...
  public parse(ev: WS.MessageEvent) {
    ...
    switch (event) {
      case 'update':
        this.emit('update', mes as GotosocialAPI.Entity.Status)
        break
      case 'notification':
        this.emit('notification', mes as GotosocialAPI.Entity.Notification)
        break
      case 'delete':
        this.emit('delete', payload)
        break
      case 'status.update':
        this.emit('status_update', mes as GotosocialAPI.Entity.Status)
        break
      default:
